...
- 数据源同步支持按标的代码过滤，但尚未实现完善的增量同步、失败重试、限流处理和接口配额管理。
- 目前没有完整的时序数据存储、行情图表、复权处理、交易日历和多市场统一代码体系。
- CSV / Excel 导入依赖固定字段格式，尚未提供字段映射 UI、清洗规则和复杂异常处理。
- 财报、研报、新闻、公告不会自动从交易所、券商或资讯平台批量抓取。

### 文档与信息解读

- PDF / Markdown / TXT 相对可用，DOC / DOCX 文档选择入口存在，但解析尚未完整实现。
...
- 持仓簿支持手动维护和导入，但交易流水尚未完整自动归集为实时持仓。
- 交易流水可以记录买入、加仓、减仓、卖出，但尚未完整反推现金余额、成本价、收益率、税费和分红。
- 仓位护栏提供单标的、行业、主题、市场、币种暴露提示，但没有组合优化、VaR、压力测试或回撤归因模型。
- 仓位护栏不会自动连接实时行情刷新，也没有后台定时风险预警任务。
